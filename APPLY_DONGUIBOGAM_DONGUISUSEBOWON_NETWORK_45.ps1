...
$bk="_backup_dongui_network45_"+(Get-Date -Format "yyyyMMdd-HHmmss")
...
foreach($s in Get-ChildItem "_payload\docs" -Recurse -File){
    ...
    if(Test-Path $dest){
        Copy-Item $dest (Join-Path $bk ($rel.Replace("\","_"))) -Force
    }
...
Write-Host "DONGUIBOGAM DONGUISUSEBOWON NETWORK 45 COMPLETE" -ForegroundColor Green
Write-Host ("Updated pages: "+$count) -ForegroundColor Cyan
Write-Host "Duplicate detail layers consolidated; independent clinical maps deepened." -ForegroundColor Cyan
