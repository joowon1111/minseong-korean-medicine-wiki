...
try{
 ...
 $bk="_backup_evidence56_"+(Get-Date -Format "yyyyMMdd-HHmmss");New-Item -ItemType Directory -Force $bk|Out-Null
 ...
 $items=@(
  @((FindBest @("migraine.md","*migraine*.md") @("\\conditions\\","\\symptoms\\")),"_blocks\migraine.md","<!-- MS56_MIGRAINE_START -->","<!-- MS56_MIGRAINE_END -->","migraine"),
  @((FindBest @("headache.md") @("\\conditions\\","\\symptoms\\")),"_blocks\headache.md","<!-- MS56_HEADACHE_START -->","<!-- MS56_HEADACHE_END -->","headache"),
  @((FindBest @("acupuncture-evidence.md") @("\\evidence-integrated\\","\\evidence")),"_blocks\acupuncture-evidence.md","<!-- MS56_ACU_STATE_START -->","<!-- MS56_ACU_STATE_END -->","acupuncture evidence"),
  @((FindBest @("*guideline*.md","*clinical-guideline*.md") @("\\evidence","\\guide")),"_blocks\guideline.md","<!-- MS56_GUIDELINE_START -->","<!-- MS56_GUIDELINE_END -->","guideline")
 )
 $updated=0;$report=@("# 56 Evidence Propagation Report","")
 foreach($i in $items){if($i[0]){if(Patch $i[0] $i[1] $i[2] $i[3]){$updated++;$report+=("- "+$i[4]+": "+$i[0])}}else{$report+=("- "+$i[4]+": SKIP")}}
 New-Item -ItemType Directory -Force "_quality_audit_56"|Out-Null;$report|Set-Content "_quality_audit_56\56_EVIDENCE_PROPAGATION_REPORT.md" -Encoding UTF8
 Write-Host "EVIDENCE PROPAGATION 56 COMPLETE" -ForegroundColor Green
