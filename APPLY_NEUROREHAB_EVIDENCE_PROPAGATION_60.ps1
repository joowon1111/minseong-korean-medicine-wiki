...
try{
 ...
 $bk="_backup_neuro60_"+(Get-Date -Format "yyyyMMdd-HHmmss");New-Item -ItemType Directory -Force $bk|Out-Null
 ...
 $items=@(
  @((FindBest @("*post*stroke*dysphagia*.md","*stroke*dysphagia*.md","*dysphagia*.md") @("\\conditions\\","\\symptoms\\","\\neuro","\\rehab")),"_blocks\stroke-dysphagia.md","<!-- MS60_PSD_START -->","<!-- MS60_PSD_END -->","post-stroke dysphagia"),
  @((FindBest @("*parkinson*.md") @("\\conditions\\","\\neuro","\\symptoms\\")),"_blocks\parkinson.md","<!-- MS60_PD_START -->","<!-- MS60_PD_END -->","Parkinson"),
  @((FindBest @("*neurolog*.md","*neuro*rehab*.md","*stroke*.md") @("\\pillar\\","\\conditions\\","\\rehab")),"_blocks\neuro-hub.md","<!-- MS60_NEURO_START -->","<!-- MS60_NEURO_END -->","neuro hub")
 )
 $updated=0;$report=@("# 60 Neurorehabilitation Evidence Report","")
 foreach($i in $items){if($i[0]){if(Patch $i[0] $i[1] $i[2] $i[3]){$updated++;$report+=("- "+$i[4]+": "+$i[0])}}else{$report+=("- "+$i[4]+": SKIP")}}
 New-Item -ItemType Directory -Force "_quality_audit_60"|Out-Null;$report|Set-Content "_quality_audit_60\60_NEURO_EVIDENCE_REPORT.md" -Encoding UTF8
 Write-Host "NEUROREHABILITATION EVIDENCE PROPAGATION 60 COMPLETE" -ForegroundColor Green
