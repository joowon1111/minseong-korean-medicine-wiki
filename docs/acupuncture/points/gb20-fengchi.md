---
title: 풍지(風池) GB20
description: 풍지(風池) GB20의 WHO 표준 위치, 전통적 효능, 주요 임상 활용과 관련 지식망을 정리합니다.
tags: [침구학, 경혈, 풍지]
status: 검토완료
last_reviewed: 2026-08-19
---
# 풍지(風池) GB20


<!-- MS_ACUPOINT_ATLAS_43_START -->
## 표준 위치·취혈

| 항목 | 내용 |
|---|---|
| WHO 코드 | **GB20** |
| 한글명 | **풍지** |
| 한자 | **風池** |
| 소속 경맥 | **족소양담경** |
| 특정혈 | **일반 경혈 — 소속 경맥·부위·배혈 맥락에서 해석** |

### WHO 표준 기반 위치

목 뒤부위, 뒤통수뼈(occipital bone)의 아래쪽, 목빗근이 닿는 곳(insertion of sternocleidomastiod muscle)의 뒤쪽 가장자리와 등 세모근이 이는 곳(origin of trapezius muscle)의 바깥쪽 가장자리 사이의 오목한 곳.

### 실제 취혈 포인트

골성·근건·피부 표지와 인접 경혈의 상대 위치를 함께 확인해 취혈합니다.

## 특정혈·임상 포인트

**일반 경혈 — 소속 경맥·부위·배혈 맥락에서 해석**

<!-- ACUPOINT_CLINICAL_START -->
**현대 연구에서 다루어진 분야**

- **임상시험에서 사용된 분야:** 고혈압 [출처 1](https://pmc.ncbi.nlm.nih.gov/articles/PMC7564320/#jcm-09-03027-t001)
- **자료의 의미:** 경혈 선택과 질환의 관련성을 분석한 자료이며, 해당 혈의 치료 효과를 평가한 결과는 아닙니다.

<details markdown="1">
<summary>출처·원문과 해석 범위</summary>

1. **임상시험의 경혈 사용 양상 분석** — Hwang YC·Lee IS·Ryu Y·Lee YS·Chae Y. [Identification of Acupoint Indication from Reverse Inference: Data Mining of Randomized Controlled Clinical Trials](https://pmc.ncbi.nlm.nih.gov/articles/PMC7564320/#jcm-09-03027-t001). Journal of Clinical Medicine. 2020;9(9):3027. 경희대학교·한국한의학연구원·가천대학교. **표 1·고찰의 한계**. DOI: 10.3390/jcm9093027.

**표준 정보:** [KMCRIC 풍지(風池) GB20](https://www.kmcric.com/database/acupoint/GB/GB20)에서 위치·취혈 정보를 확인할 수 있습니다. 혈명·코드·소속 경맥을 대조했으며, 이 링크를 주치·효능의 근거로 사용하지 않습니다.

**위치 데이터:** [KM-Agent 경혈 자료](https://github.com/wonyung-lee/km-agent/blob/main/data/acupoints.csv), Won-Yung Lee 외 5인, 2026, [CC BY 4.0](https://github.com/wonyung-lee/km-agent/blob/main/LICENSE). 기존 위치 자료를 사용하며, 일부 누락된 혈명 한자를 KMCRIC 목록과 대조하여 수정했습니다.

논문·지침의 필요한 사실 관계를 자체 문장으로 요약했습니다. [자료별 범위와 이용조건](../../portal/acupuncture.md#sources)을 함께 확인하세요.

</details>

**임상 평가·배혈 연결**

안정 시 혈압과 복용 약물을 확인하고, 심한 두통·흉통·신경학적 증상은 즉시 원인 평가로 연결합니다. [평가 자료](../../acupuncture-integrated/safety.md)

임상 평가는 아카이브의 평가·기록 안내입니다.


<!-- ACUPOINT_CLINICAL_END -->

## 자침법·안전

- 直刺 0.3～1寸 (반대쪽 眼球 방향으로 刺入)

- 斜刺 0.5～1.5寸 (약간 斜下 방향인 耳垂와 수평으로 刺入)

자침 깊이와 방향은 체형과 주변 혈관·신경·장기 위치를 고려해 조절하며 의료전문가의 판단이 필요합니다.

## 지식망 연결

- [WHO 표준 361경혈 아틀라스](../../acupoint-network/standard-atlas.md)
- [경맥→증상 임상 지도](../../acupoint-network/by-condition.md)
- [특정혈 임상 지식망](../../meridian-network/special-points/index.md)
- [임상 핵심 배혈](../../network/acupoint-combinations.md)
- [침구 임상 지식망](../../network/acupuncture-clinical-map.md)

## 현대 연구를 읽는 법

현대 침구 연구에서는 혈명뿐 아니라 표준 위치, 자침 깊이·방향, 수기·전침 조건, 유침시간, 치료 횟수와 결과지표를 함께 봅니다. WHO 표준 위치는 연구·교육에서 위치의 재현성을 높이는 기준으로 활용됩니다.

## 표준화 출처

WHO Western Pacific Region의 361 표준경혈 체계와 이를 기반으로 정리된 KMCRIC 표준경혈 DB를 기준으로 위치·취혈 구조를 맞췄습니다. 구조화 데이터는 WHO 표준 기반의 KM-Agent `acupoints.csv`(CC BY 4.0)를 사용했습니다.
<!-- MS_ACUPOINT_ATLAS_43_END -->


**경맥:** 족소양담경

## 표준 위치
뒤통수 아래, 흉쇄유돌근과 승모근 상부 사이의 오목한 곳

경혈 위치는 WHO *Standard Acupuncture Point Locations in the Western Pacific Region*의 표준화 원칙을 바탕으로 정리한다.

## 전통적 효능
**거풍통락·청두목**

## 주요 활용
### 두경부
후두통·경항부 긴장·현훈 관련 배혈에서 널리 활용
### 두면부
합곡·태충 등과 함께 두면부 증상에 배합

## 지식망 연결
- [관련 임상 지식망](../../network/acupuncture-clinical-map.md)

!!! note "임상 적용"
    실제 침구치료는 증상 하나와 경혈 하나를 일대일로 대응시키기보다 경락·장부·변증, 통증 부위, 전신 상태를 함께 고려해 배혈한다.

## 참고
- World Health Organization. *WHO Standard Acupuncture Point Locations in the Western Pacific Region.* 2008.

<!-- MINSEONG_ONE_SHOT_FIX_V2 -->
## 관련 핵심 문서

- [침구·치료 한눈에 보기](../../acupuncture-integrated/index.md)
- [증상으로 침구치료 찾기](../../acupuncture-integrated/by-symptom.md)
- [침구 안전·위험신호](../../acupuncture-integrated/safety.md)



