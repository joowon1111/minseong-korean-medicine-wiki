---
title: 명문 GV4
description: 명문 GV4의 위치, 소속 경맥, 특정혈 분류, 전통 주치, 임상 배혈, 해부학·안전성과 연구 방향을 정리합니다.
tags: [경혈, 독맥, 특정혈, 침구]
---
# 명문 GV4

<!-- MS_ACUPOINT_ATLAS_43_START -->
## 표준 위치·취혈

| 항목 | 내용 |
|---|---|
| WHO 코드 | **GV4** |
| 한글명 | **명문** |
| 한자 | **命門** |
| 소속 경맥 | **독맥** |
| 특정혈 | **일반 경혈 — 소속 경맥·부위·배혈 맥락에서 해석** |

### WHO 표준 기반 위치

허리부위, 뒤정중선(posterior median line) 위, 둘째 허리뼈 가시돌기(spinous process of the 2nd lumbar vertebra) 아래쪽 오목한 곳.

### 실제 취혈 포인트

골성·근건·피부 표지와 인접 경혈의 상대 위치를 함께 확인해 취혈합니다.

## 특정혈·임상 포인트

**일반 경혈 — 소속 경맥·부위·배혈 맥락에서 해석**

## 자침법·안전

- 直刺 0.3～0.5寸

- 斜刺 1～1.5寸 (약간 上向으로 刺入)

자침 깊이와 방향은 체형과 주변 혈관·신경·장기 위치를 고려해 조절하며 의료전문가의 판단이 필요합니다.

## 지식망 연결

- [WHO 표준 361경혈 아틀라스](../../acupoint-network/standard-atlas.md)
- [경맥→증상 임상 지도](../../acupoint-network/by-condition.md)
- [특정혈 임상 지식망](../../meridian-network/special-points/index.md)
- [임상 핵심 배혈](../../network/acupoint-combinations.md)
- [침구 임상 지식망](../../network/acupuncture-clinical-map.md)

## 현대 연구를 읽는 법

현대 침구 연구에서는 혈명뿐 아니라 표준 위치, 자침 깊이·방향, 수기·전침 조건, 유침시간, 치료 횟수와 결과지표를 함께 봅니다. WHO 표준 위치는 연구·교육에서 위치의 재현성을 높이는 기준으로 활용됩니다.

## 표준화 출처

WHO Western Pacific Region의 361 표준경혈 체계와 이를 기반으로 정리된 KMCRIC 표준경혈 DB를 기준으로 위치·취혈 구조를 맞췄습니다. 구조화 데이터는 WHO 표준 기반의 KM-Agent `acupoints.csv`(CC BY 4.0)를 사용했습니다.
<!-- MS_ACUPOINT_ATLAS_43_END -->


## 핵심 정보

| 항목 | 내용 |
|---|---|
| 소속 | 독맥 |
| 특정혈 | 독맥의 요부 대표혈 |
| 위치 요약 | 제2요추 극돌기 아래 정중선 |
| 임상 연결 | 요부·신허·회복·냉감 관련 전통 연결 |

## 위치와 취혈

제2요추 극돌기 아래 정중선를 기준으로 취혈합니다. 실제 임상에서는 체형 차이와 촉진, 뼈·근육·건·혈관·신경 등 해부학적 표지를 함께 확인합니다.

## 전통적 의미와 주치

<!-- ACUPOINT_CLINICAL_START -->
**전통적 효능·주치**

- **대표 주치·용도:** 요통 · 하지 무력감 · 월경불순 [전통 주치·교육자료](https://www.americandragon.com/Points/DU-4.html)
- **효능·활용 방향:** 이요 — 허리의 통증·기능을 다루는 방향; 안신 — 긴장·수면 불편을 조절

**현대 임상 활용**

- **만성요통 지침의 배혈 (2020년판):** 성인 만성 비특이적 요통의 일반침치료에서 기본 혈위로 제시된 혈입니다. 공개 원문을 확인한 2020년판의 배혈을 소개합니다. [출처 1](https://nikom.or.kr/nckm/module/practiceGuide/view.do?guide_idx=149&menu_idx=14)

진료지침의 배혈에서 맡는 역할입니다. 단일혈의 독립적인 효과를 뜻하지 않습니다.

<details markdown="1">
<summary>출처·원문과 해석 범위</summary>

1. **임상진료지침의 일반침 배혈·확인한 판본** — 대한침구의학회 지침 개발진. [만성요통증후군 한의표준임상진료지침 (2020년판)](https://nikom.or.kr/nckm/module/practiceGuide/view.do?guide_idx=149&menu_idx=14). 한국한의약진흥원 한의표준임상진료지침 개발사업단. 2020. 대한침구의학회·한국한의약진흥원; 국가한의임상정보포털. **67쪽(PDF 77쪽), 일반침 임상적 고려사항**. [이용조건](https://nikom.or.kr/nckm/html.do?menu_idx=95). [해당 쪽 PDF](https://nikom.or.kr/nckm/module/practiceGuide/download.do?guide_idx=149&file_type=pdf#page=77).

**표준 정보:** [KMCRIC 명문 GV4](https://www.kmcric.com/database/acupoint/GV/GV4)에서 위치·취혈 정보를 확인할 수 있습니다. 혈명·코드·소속 경맥을 대조했으며, 이 링크를 주치·효능의 근거로 사용하지 않습니다.

**위치 데이터:** [KM-Agent 경혈 자료](https://github.com/wonyung-lee/km-agent/blob/main/data/acupoints.csv), Won-Yung Lee 외 5인, 2026, [CC BY 4.0](https://github.com/wonyung-lee/km-agent/blob/main/LICENSE). 기존 위치 자료를 사용하며, 일부 누락된 혈명 한자를 KMCRIC 목록과 대조하여 수정했습니다.

논문·지침의 필요한 사실 관계를 자체 문장으로 요약했습니다. [자료별 범위와 이용조건](../../portal/acupuncture.md#sources)을 함께 확인하세요.

</details>

**임상 평가·배혈 연결**

허리 움직임·앉기·걷기와 통증 분포를 기록하고, 방사통은 근력·감각 평가를 함께 봅니다. [평가 자료](../../conditions/low-back-pain.md)

임상 평가는 아카이브의 평가·기록 안내입니다.


<!-- ACUPOINT_CLINICAL_END -->

## 자주 연결하는 배혈

**신수 BL23·관원 CV4**

배혈은 주증과 동반 증상에 따라 국소혈·원위혈·특정혈을 조합하고, 필요에 따라 침·전침·뜸 등 자극 방식을 달리합니다.

## 특정혈 체계에서 보기

이 혈의 **독맥의 요부 대표혈** 성격은 같은 경맥의 오수혈·원혈·락혈·극혈, 표리경 및 기경팔맥 관계와 함께 보면 더 잘 이해할 수 있습니다.

→ [특정혈 임상 지식망](../../meridian-network/special-points/index.md)

## 해부학과 안전

척추관 방향의 과도한 깊이를 피하고 요추 해부학을 확인합니다.

경혈 위치 정보는 교육용이며 자침은 해부학과 침구 안전교육을 받은 의료전문가의 판단이 필요합니다.

## 현대 연구

현대 침 연구에서는 통증, 기능, 호흡·소화·수면·자율신경 등 해당 임상 영역의 증상 점수와 기능지표를 대상으로 단일혈 또는 복합 배혈의 임상 효과와 생리학적 기전이 연구되고 있습니다. 실제 연구를 해석할 때에는 사용한 경혈 조합, 자극 방식, 치료 빈도, 비교군과 평가변수를 함께 확인합니다.

## 관련 문서

- [경혈 임상 지식망](../../acupoint-network/index.md)
- [경락·경맥 지식망](../../meridian-network/index.md)
- [임상 핵심 배혈 조합](../../network/acupoint-combinations.md)
- [현대 임상근거](../../pillar/clinical-evidence.md)


