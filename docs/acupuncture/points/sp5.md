---
title: 상구 SP5
description: 상구 SP5 표준 위치·취혈·임상 지식망
status: 검토완료
last_reviewed: 2026-08-22
---
# 상구 SP5

<!-- MS_ACUPOINT_ATLAS_43_START -->
## 표준 위치·취혈

| 항목 | 내용 |
|---|---|
| WHO 코드 | **SP5** |
| 한글명 | **상구** |
| 한자 | **商丘** |
| 소속 경맥 | **족태음비경** |
| 특정혈 | **오수혈·경혈** |

### WHO 표준 기반 위치

발 안쪽면, 안쪽복사(medial malleolus) 아래앞쪽, 발배뼈거친면(tuberosity of the navicular bone)과 안쪽복사 융기(prominence of the medial malleolus)를 연결하는 선 중간의 오목한 곳이며, 中封(LR4)의 뒤쪽, 照海(KI6)의 앞쪽에 위치한다.

### 실제 취혈 포인트

안쪽복사 앞모서리를 지나는 수직선과 아래모서리를 지나는 수평선이 만나는 지점에서 취혈한다.

## 특정혈·임상 포인트

**오수혈·경혈**

<!-- ACUPOINT_CLINICAL_START -->
| 항목 | 내용 |
|---|---|
| 대표 주치·용도 | 발목 통증 · 복부팽만 · 설사 |
| 효능의 전통적 설명 | 서근 — 근육·힘줄의 긴장을 조절; 건비 — 비위의 소화·운화 기능을 보완 |
| 임상 평가·활용 | 목 회전·굽힘에 따른 통증과 팔의 감각·근력을 함께 비교합니다. [평가 자료](../../conditions/neck-pain.md) |
| 주치 출처 | [상구 전통 주치·교육자료](https://www.americandragon.com/Points/SP-5.html) |
| 사암침법에서의 역할 | [비승격 사혈](../../acupuncture-specific/saam-12-meridians.md#sp) |

주치와 효능은 전통적 활용을 요약한 것입니다. 실제 치료에서는 증상·기능과 배혈 전체를 평가합니다.
<!-- ACUPOINT_CLINICAL_END -->

## 자침법·안전

- 0.3～0.5寸 直刺를 하거나

- 鍼尖을 解谿(ST41)를 향해 1～1.5寸  橫刺

자침 깊이와 방향은 체형과 주변 혈관·신경·장기 위치를 고려해 조절하며 의료전문가의 판단이 필요합니다.

## 지식망 연결

- [WHO 표준 361경혈 아틀라스](../../acupoint-network/standard-atlas.md)
- [경맥→증상 임상 지도](../../acupoint-network/by-condition.md)
- [특정혈 임상 지식망](../../meridian-network/special-points/index.md)
- [임상 핵심 배혈](../../network/acupoint-combinations.md)
- [침구 임상 지식망](../../network/acupuncture-clinical-map.md)

## 현대 연구를 읽는 법

현대 침구 연구에서는 혈명뿐 아니라 표준 위치, 자침 깊이·방향, 수기·전침 조건, 유침시간, 치료 횟수와 결과지표를 함께 봅니다. WHO 표준 위치는 연구·교육에서 위치의 재현성을 높이는 기준으로 활용됩니다.

## 표준화 출처

WHO Western Pacific Region의 361 표준경혈 체계와 이를 기반으로 정리된 KMCRIC 표준경혈 DB를 기준으로 위치·취혈 구조를 맞췄습니다. 구조화 데이터는 WHO 표준 기반의 KM-Agent `acupoints.csv`(CC BY 4.0)를 사용했습니다.
<!-- MS_ACUPOINT_ATLAS_43_END -->

