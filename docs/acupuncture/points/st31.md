---
title: 비관 ST31
description: 비관 ST31 표준 위치·취혈·임상 지식망
status: 검토완료
last_reviewed: 2026-08-22
---
# 비관 ST31

<!-- MS_ACUPOINT_ATLAS_43_START -->
## 표준 위치·취혈

| 항목 | 내용 |
|---|---|
| WHO 코드 | **ST31** |
| 한글명 | **비관** |
| 한자 | **髀關** |
| 소속 경맥 | **족양명위경** |
| 특정혈 | **일반 경혈 — 소속 경맥·부위·배혈 맥락에서 해석** |

### WHO 표준 기반 위치

넓적다리 앞쪽면, 넙다리곧은근(rectus femoris muscle) 몸쪽끝, 넙다리빗근(sartorius muscle), 넙다리근막긴장근(tensor fasciae latae muscle)의 세 근육 사이의 오목한 곳.

### 실제 취혈 포인트

엉덩이와 무릎을 약간 굽히고, 넓적다리의 앞안쪽에 저항을 주며 다리를 살짝 외전시키면 세모꼴의 오목한 부위가 나타난다. 넙다리곧은근의 몸쪽끝은 안쪽의 넙다리빗근과 가쪽의 넙다리근막긴장근 사이의 오목한 곳에서 나타난다. 髀關(ST31)은 이 세모의 꼭짓점아래 가장 오목한 곳에 위치한다. 무릎뼈바닥 가쪽끝(lateral end of the base of the patella)과 위앞엉덩뼈가시(anterior superior iliac spine)의 연결선에서 두덩결합의 아래모서리(inferior border of the pubic symphysis)와 같은 높이에서 취혈한다.

## 특정혈·임상 포인트

**일반 경혈 — 소속 경맥·부위·배혈 맥락에서 해석**

<!-- ACUPOINT_CLINICAL_START -->
| 항목 | 내용 |
|---|---|
| 대표 주치·용도 | 고관절 부위 통증 · 하지 통증 |
| 효능의 전통적 설명 | 통경 — 경맥의 통증·불편을 조절; 지통 — 통증을 완화하는 방향 |
| 임상 평가·활용 | 고관절 움직임·보행과 둔부에서 하지로 이어지는 통증 분포를 확인합니다. [평가 자료](../../clinical-anatomy/mps-hip.md) |
| 주치 출처 | [비관 전통 주치·교육자료](https://www.americandragon.com/Points/ST-31.html) |

주치와 효능은 전통적 활용을 요약한 것입니다. 실제 치료에서는 증상·기능과 배혈 전체를 평가합니다.
<!-- ACUPOINT_CLINICAL_END -->

## 자침법·안전

- 直刺 0.8～1.5寸

- 斜刺 1.5寸～2寸

자침 깊이와 방향은 체형과 주변 혈관·신경·장기 위치를 고려해 조절하며 의료전문가의 판단이 필요합니다.

## 지식망 연결

- [WHO 표준 361경혈 아틀라스](../../acupoint-network/standard-atlas.md)
- [경맥→증상 임상 지도](../../acupoint-network/by-condition.md)
- [특정혈 임상 지식망](../../meridian-network/special-points/index.md)
- [임상 핵심 배혈](../../network/acupoint-combinations.md)
- [침구 임상 지식망](../../network/acupuncture-clinical-map.md)

## 현대 연구를 읽는 법

현대 침구 연구에서는 혈명뿐 아니라 표준 위치, 자침 깊이·방향, 수기·전침 조건, 유침시간, 치료 횟수와 결과지표를 함께 봅니다. WHO 표준 위치는 연구·교육에서 위치의 재현성을 높이는 기준으로 활용됩니다.

## 표준화 출처

WHO Western Pacific Region의 361 표준경혈 체계와 이를 기반으로 정리된 KMCRIC 표준경혈 DB를 기준으로 위치·취혈 구조를 맞췄습니다. 구조화 데이터는 WHO 표준 기반의 KM-Agent `acupoints.csv`(CC BY 4.0)를 사용했습니다.
<!-- MS_ACUPOINT_ATLAS_43_END -->

