---
title: 간사 PC5
description: 간사 PC5 표준 위치·취혈·임상 지식망
status: 검토완료
last_reviewed: 2026-08-22
---
# 간사 PC5

<!-- MS_ACUPOINT_ATLAS_43_START -->
## 표준 위치·취혈

| 항목 | 내용 |
|---|---|
| WHO 코드 | **PC5** |
| 한글명 | **간사** |
| 한자 | **間使** |
| 소속 경맥 | **수궐음심포경** |
| 특정혈 | **오수혈·경혈** |

### WHO 표준 기반 위치

아래팔 앞쪽면, 긴손바닥근힘줄(palmaris longus tendon)과 노쪽손목굽힘근힘줄(flexor carpi radialis tendon)의 사이, 손바닥 쪽 손목주름(palmar wrist crease) 에서 몸 쪽으로 3寸.

### 실제 취혈 포인트

주먹을 가볍게 움켜쥐고 손바닥이 위쪽을 향하도록 손목을 돌린채 팔꿈치를 약간 굽히면, 긴손바닥근힘줄과 노쪽손목굽힘근힘줄이 현저하게 드러난다. 大陵(PC7)에서 몸 쪽으로 3寸 지점에서 취혈한다. 긴손바닥근힘줄이 보이지 않는다면, 노쪽손목굽힘근힘줄 자쪽에서 취혈한다.

## 특정혈·임상 포인트

**오수혈·경혈**

<!-- ACUPOINT_CLINICAL_START -->
| 항목 | 내용 |
|---|---|
| 대표 주치·용도 | 두근거림 · 흉부 통증·답답함 · 구토·헛구역 |
| 효능의 전통적 설명 | 안신 — 긴장·수면 불편을 조절; 화위 — 위장 불편과 상역을 조절 |
| 임상 평가·활용 | 잠드는 시간·중간 각성·낮 기능과 두근거림의 발생 상황을 기록합니다. [평가 자료](../../conditions/insomnia.md) |
| 주치 출처 | [간사 전통 주치·교육자료](https://www.americandragon.com/Points/PC-5.html) |

주치와 효능은 전통적 활용을 요약한 것입니다. 실제 치료에서는 증상·기능과 배혈 전체를 평가합니다.
<!-- ACUPOINT_CLINICAL_END -->

## 자침법·안전

- 直刺 0.5～1寸

- 斜刺 1～1.5寸

- 軀幹疾患 치료시는 약간 橈骨側으로 上向하여 1～1.5寸 斜刺

자침 깊이와 방향은 체형과 주변 혈관·신경·장기 위치를 고려해 조절하며 의료전문가의 판단이 필요합니다.

## 지식망 연결

- [WHO 표준 361경혈 아틀라스](../../acupoint-network/standard-atlas.md)
- [경맥→증상 임상 지도](../../acupoint-network/by-condition.md)
- [특정혈 임상 지식망](../../meridian-network/special-points/index.md)
- [임상 핵심 배혈](../../network/acupoint-combinations.md)
- [침구 임상 지식망](../../network/acupuncture-clinical-map.md)

## 현대 연구를 읽는 법

현대 침구 연구에서는 혈명뿐 아니라 표준 위치, 자침 깊이·방향, 수기·전침 조건, 유침시간, 치료 횟수와 결과지표를 함께 봅니다. WHO 표준 위치는 연구·교육에서 위치의 재현성을 높이는 기준으로 활용됩니다.

## 표준화 출처

WHO Western Pacific Region의 361 표준경혈 체계와 이를 기반으로 정리된 KMCRIC 표준경혈 DB를 기준으로 위치·취혈 구조를 맞췄습니다. 구조화 데이터는 WHO 표준 기반의 KM-Agent `acupoints.csv`(CC BY 4.0)를 사용했습니다.
<!-- MS_ACUPOINT_ATLAS_43_END -->

