---
title: 음릉천 SP9
description: 음릉천 SP9의 임상 활용, 배혈, 안전성과 관련 연구를 정리합니다.
tags: [경혈, 임상한의학]
---
# 음릉천 SP9


<!-- MS_ACUPOINT_ATLAS_43_START -->
## 표준 위치·취혈

| 항목 | 내용 |
|---|---|
| WHO 코드 | **SP9** |
| 한글명 | **음릉천** |
| 한자 | **陰陵泉** |
| 소속 경맥 | **족태음비경** |
| 특정혈 | **오수혈·합혈** |

### WHO 표준 기반 위치

종아리 정강뼈면, 정강뼈 안쪽관절융기 아래모서리와 정강뼈 안쪽모서리(medial border of the tibia) 사이의 오목한 곳.

### 실제 취혈 포인트

손가락을 정강뼈 안쪽모서리를 따라서 몸 쪽으로 움직이면 무릎관절 아래에 오목한 곳이 느껴진다. 정강뼈 안쪽관절융기의 아래모서리와 정강뼈 뒤모서리로 이루어진 각의 오목한 곳에서 취혈한다.

## 특정혈·임상 포인트

**오수혈·합혈**

<!-- ACUPOINT_CLINICAL_START -->
| 항목 | 내용 |
|---|---|
| 대표 주치·용도 | 부종 · 배뇨 불편 · 무릎 통증·불편 |
| 활용 방향 | 부종의 완화와 배뇨 불편의 조절 |
| 임상 평가·활용 | 부종의 좌우·시간대·체중 변화와 동반 증상을 확인합니다. [평가 자료](../../acupuncture-integrated/followup.md) |
| 주치 출처 | [음릉천 전통 주치·교육자료](https://www.americandragon.com/Points/SP-9.html) |

주치와 효능은 전통적 활용을 요약한 것입니다. 실제 치료에서는 증상·기능과 배혈 전체를 평가합니다.
<!-- ACUPOINT_CLINICAL_END -->

## 자침법·안전

- 直刺 0.5～1寸

- 斜刺 1～1.5寸

- 陽陵泉(GB34)으로 透刺하기도 한다.

자침 깊이와 방향은 체형과 주변 혈관·신경·장기 위치를 고려해 조절하며 의료전문가의 판단이 필요합니다.

## 지식망 연결

- [WHO 표준 361경혈 아틀라스](../../acupoint-network/standard-atlas.md)
- [경맥→증상 임상 지도](../../acupoint-network/by-condition.md)
- [특정혈 임상 지식망](../../meridian-network/special-points/index.md)
- [임상 핵심 배혈](../../network/acupoint-combinations.md)
- [침구 임상 지식망](../../network/acupuncture-clinical-map.md)

## 현대 연구를 읽는 법

현대 침구 연구에서는 혈명뿐 아니라 표준 위치, 자침 깊이·방향, 수기·전침 조건, 유침시간, 치료 횟수와 결과지표를 함께 봅니다. WHO 표준 위치는 연구·교육에서 위치의 재현성을 높이는 기준으로 활용됩니다.

## 표준화 출처

WHO Western Pacific Region의 361 표준경혈 체계와 이를 기반으로 정리된 KMCRIC 표준경혈 DB를 기준으로 위치·취혈 구조를 맞췄습니다. 구조화 데이터는 WHO 표준 기반의 KM-Agent `acupoints.csv`(CC BY 4.0)를 사용했습니다.
<!-- MS_ACUPOINT_ATLAS_43_END -->


## 임상에서 보는 포인트
**수습·부종·무릎** 관련 증상에서 다른 경혈과 함께 배혈하는 경우가 많습니다.

## 취혈·배혈
정확한 해부학적 표지를 기준으로 취혈하며, 증상 위치뿐 아니라 경락·병증·근육과 관절 상태를 함께 보고 배혈합니다.

## 해부학·안전
자침 깊이와 방향은 주변 신경·혈관 및 장기 등 해부학적 구조를 고려합니다.

## 현대 연구
침 연구에서는 통증과 기능, 소화·수면·자율신경 등 다양한 임상 결과와 생리학적 기전이 연구되고 있습니다.

## 연결 문서
- [경혈·경락](../../acupuncture-integrated/index.md)
- [침구·한의치료 허브](../../pillar/acupuncture-treatment.md)


