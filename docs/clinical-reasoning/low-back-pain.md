---
title: 요통의 한의학적 임상 추론
description: 요통을 급만성·통증양상·어혈·풍한습비·허손으로 구분하고 침구·독활기생탕·현대근거로 연결합니다.
tags: [임상추론, 변증, 치법, 지식망]
status: 검토완료
last_reviewed: 2026-08-19
---
# 요통의 한의학적 임상 추론

## 1. 먼저 보는 것
통증 위치·발병 시점·움직임과의 관계·하지 증상·기능 제한 등을 살핀다.

## 2. 변증
```text
요통
├─ 고정·오래된 통증 → 어혈 고려
├─ 냉·습과 연동 → 풍한습비
└─ 만성 + 허약 → 간신·기혈허
```

## 3. 치료
국소·경락 치료:
- [침](../treatments/acupuncture.md)
- [약침](../treatments/pharmacopuncture.md)
- [전침](../treatments/electroacupuncture.md)
- [부항](../treatments/cupping.md)

경혈:
- [신수 BL23](../acupuncture/points/bl23-shenshu.md)
- [위중 BL40](../acupuncture/points/bl40-weizhong.md)

방제:
→ [독활기생탕](../formulas/duhuo-jisheng-tang.md)

→ [요통 근거 카드](../authority/low-back-pain.md)
