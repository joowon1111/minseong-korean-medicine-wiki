...
description: 민성 한의학 아카이브의 주제별 탐색, 문서별 JSON·지식그래프, 대표 URL과 출처를 연결하는 AI·검색엔진용 정보구조를 설명합니다.
...
사람이 읽는 [AI 핵심 색인](../ai-index.md)과 함께 다음 자료를 제공합니다.

| 자료 | 역할 |
|---|---|
| [llms.txt](../llms.txt) | 핵심 허브·출처와 기계가 읽는 자료의 진입점 |
| [문서별 JSON 색인](https://wiki.minseong.co.kr/assets/ai/page-index.json) | 주제별 문서와 해당 문서의 JSON 주소를 찾는 목록 |
| [데이터 안내](https://wiki.minseong.co.kr/assets/ai/knowledge-manifest.json) | 지식 데이터의 파일 구성과 접근 경로 |
| [지식그래프](https://wiki.minseong.co.kr/assets/ai/knowledge-graph.json) | 문서의 유형과 내부 연결 관계를 모아 둔 자료 |
| [사이트맵](https://wiki.minseong.co.kr/sitemap.xml)·[robots.txt](../robots.txt) | 공개 페이지의 발견과 크롤링 지침 |

한 주제를 읽을 때는 문서별 색인에서 해당 JSON을 찾으면 됩니다. 문서별 JSON에는 공개 본문, 대표 HTML URL, 실제 제목 앵커와 관련 링크가 들어 있으며 HTML과 함께 생성됩니다. **인용할 때는 JSON 주소보다 대표 HTML 주소와 해당 제목 앵커를 사용**하고, 연구 결과는 연결된 원문 출처까지 확인합니다.

지식그래프의 연결은 ‘관련 문서로 이동할 수 있다’는 의미입니다. 연결 자체가 임상적 동등성이나 특정 치료의 추천을 뜻하지 않습니다.
...
## 현재 robots.txt {#robotstxt}

공개된 [robots.txt](../robots.txt)는 아래처럼 일반 크롤러의 접근을 허용하고 사이트맵을 안내합니다. 실제 수집·색인 여부는 크롤러와 호스팅의 응답 상태 등에 따라 달라지므로, 이 설정만으로 모든 검색엔진과 AI가 내용을 수집했다고 판단하지 않습니다.
...
아래의 AEO·GEO 설명은 질문에 직접 답하고 주제의 관계와 출처를 명확히 제공한다는 뜻입니다. 특정 AI가 요구하는 전용 규격을 뜻하지 않습니다. Google AI 검색도 기존 SEO 원칙을 바탕으로 하며, 별도 AI 파일이나 스키마를 필수로 요구하지 않습니다. [Google 공식 안내](https://developers.google.com/search/docs/appearance/ai-features).
