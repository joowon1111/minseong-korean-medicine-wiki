---
title: 자감초탕(炙甘草湯)
description: 기·혈·음의 허손과 심계·맥결대를 함께 살피는 상한론 대표 보익방의 구성·방의·임상 감별을 정리합니다.
tags: [방제학, 자감초탕, 복맥탕, 기혈음허, 심계]
status: 검토완료
last_reviewed: '2026-09-16'
---
# 자감초탕(炙甘草湯)

자감초탕은 『상한론』의 **“맥결대(脈結代), 심동계(心動悸)”** 조문으로 잘 알려진 처방이며, **복맥탕(復脈湯)**이라고도 합니다. 심계와 불규칙한 맥이 기·혈·음의 소모, 진액부족과 함께 나타나는 허손을 다루는 대표방입니다.

## 한눈에 보기

| 항목 | 핵심 |
|---|---|
| 출전 | 『상한론』 |
| 다른 이름 | 복맥탕 |
| 구성 | 자감초·생지황·인삼·맥문동·아교·마자인·계지·생강·대조 |
| 중심 치법 | 익기자음·양혈복맥·통양 |
| 살필 단서 | 심계·맥의 불규칙 + 피로·숨참·구갈·건조·허약 |

## 처방 구조

| 약재군 | 역할 |
|---|---|
| 자감초·인삼·대조 | 기를 보하고 중초를 도우며 맥을 회복하는 기반 |
| 생지황·맥문동·아교·마자인 | 혈과 음·진액을 길러 건조와 소모를 보완 |
| 계지·생강 | 양기를 통하게 하고 영위와 맥의 흐름을 조화 |

보익약만 모은 처방이 아니라 **기와 음혈을 채우면서 계지·생강으로 순환을 돕는 구조**입니다. 그래서 단순 피로보다 심계·맥상·숨참과 건조한 허손이 함께 있는지가 중요합니다.

## 원방의 수치·법제와 전탕

| 확인점 | 원방을 읽을 때 구분할 내용 |
|---|---|
| 자감초 | 생감초가 아니라 `炙甘草`로 기록된 포제품인지 확인 |
| 생지황 | 숙지황으로 자동 치환하지 않고 원방의 높은 상대비를 보존 |
| 아교 | 다른 약재를 달인 뒤 녹여 넣는 용화법과 실제 원료 확인 |
| 마자인 | 종자 원료·분쇄 여부와 산패·보관 상태 확인 |
| 전탕 매체 | 원문에 보이는 청주와 물의 사용을 현대 탕제에서 어떻게 재현·변경했는지 기록 |

고전 단위를 임의로 g으로 바꾸거나 현대 엑스제의 함량과 같은 숫자로 읽지 않습니다. 청주를 쓴 원방과 알코올을 사용하지 않는 현대 조제는 제법 차이를 명시합니다.

## 임상에서 살피는 모습

- 병후·과로·진액소모 뒤 가슴이 두근거리고 맥이 고르지 않음
- 쉽게 숨차고 피곤하며 입과 목이 마름
- 안색저하·불면·불안감이 허약과 함께 나타남
- 마른기침이나 변비 등 건조한 양상이 동반
- 땀·설사·출혈 또는 장기 질환 뒤 회복이 더딤

## 비슷한 처방과 감별

| 처방 | 중심 차이 |
|---|---|
| [생맥산](shengmai-san.md) | 기음양허·땀·갈증·숨참을 세 약재로 간결하게 다룸 |
| **자감초탕** | 심계·맥결대와 기혈음 허손을 더 폭넓게 봄 |
| [귀비탕](guibi-tang.md) | 심비양허에 불면·건망·걱정·식욕저하가 중심 |
| [천왕보심단](tianwang-buxin-dan.md) | 심신음혈부족·허열·불면의 비중이 큼 |
| [당귀보혈탕](danggui-buxue-tang.md) | 황기 중심의 보기생혈, 출혈·병후 기혈회복 |
| [팔물탕](bazhen-tang.md) | 기허와 혈허의 전반적인 기혈쌍보 |

## 심계는 먼저 확인합니다

심계나 불규칙맥은 빈혈·갑상선·전해질 이상·부정맥·약물 등 여러 원인이 있을 수 있습니다. 증상이 반복되면 맥박 기록, 심전도와 필요한 검사를 통해 원인을 확인합니다. 흉통·실신·호흡곤란, 새로 발생한 빠르거나 매우 느린 맥은 즉시 의료기관 평가가 필요합니다.

## 안전하게 보기

감초가 포함되므로 장기·고용량 복용 시 혈압상승·부종·저칼륨혈증 가능성을 살피고, 이뇨제·스테로이드·디곡신 등 복용약을 확인합니다. 아교 등 자윤한 약재가 소화에 부담을 주는지, 임신·수유 및 항응고제 복용 여부도 함께 봅니다.

## 출전·참고

- 『상한론』 변태양병맥증병치
- [대한한방내과학회지: 자감초탕과 침치료로 호전된 서맥성 부정맥 환자 증례](https://jikm.or.kr/upload/pdf/201004/g201003_06.pdf)

## 관련 문서

- [보익·피로·회복 한의학 허브](../pillar/tonic-recovery.md)
- [수면·정서·신경 허브](../pillar/sleep-neuro.md)
- [일반 방제 임상 지도](../herbal-integrated/general-formulary.md)
