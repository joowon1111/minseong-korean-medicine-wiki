---
title: 편두통 — 현대 임상근거 카드
description: 편두통에서 침 관련 치료의 systematic review·network meta-analysis와 두면부·경혈 지식망을 연결합니다.
tags: [편두통, 침, 임상근거, network-meta-analysis]
status: 검토완료
last_reviewed: 2026-08-19
---
# 편두통 — 현대 임상근거 카드

편두통은 반복되는 두통과 오심, 빛·소리에 대한 과민 등이 동반될 수 있는 신경계 질환이다. 한의학에서는 통증의 위치·성질과 동반증상을 살펴 경락·기혈·담·풍 등의 구조를 함께 탐색할 수 있다.

## 2024 systematic review·network meta-analysis
2024년 *Journal of Pain Research*에 발표된 systematic review/network meta-analysis는 편두통에서 여러 침 관련 치료법의 효과를 비교했다.

**PMID:** `38505499`  
**PMCID:** `PMC10950004`  
**DOI:** `10.2147/JPR.S452971`

## 2025 systematic review·meta-analysis
2025년 발표된 meta-analysis에는 **23개 RCT, 2,295명**이 포함됐다. Sham acupuncture와 비교해 편두통 지속시간, 평균 발작 횟수, 4주당 편두통 일수와 일부 삶의 질 지표에서 개선이 보고됐다.

**PMID:** `41219786`

## 침구 지식망
- [합곡 LI4](../../acupoint-network/li4.md)
- [백회 GV20](../../acupoint-network/gv20.md)
- [수양명대장경](../../meridian-network/large-intestine-meridian.md)
- [독맥](../../meridian-network/governing-vessel.md)

## 해석
편두통 침 연구는 sham acupuncture·약물치료 등 다양한 비교군을 사용하므로 연구별 치료 횟수와 경혈 구성, 예방치료와 급성기 치료 여부를 구분해서 보는 것이 좋다.

→ [한의학 근거 읽는 법](../../evidence-guide/index.md)
